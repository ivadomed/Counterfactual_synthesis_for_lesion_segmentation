...
Optionnaly, you can specify a chekpoint from which the training can start again : cfg.model.resume_from_checkpoint = path/to/model.ckpt
...
To train the diffusion model in the latent space of the previously trained VQ-GAN model, you need to run the following command
```
python train/train_ddpm.py model=ddpm dataset=default dataset.root_dir=<INSERT_PATH_TO_ROOT_DIRECTORY> model.results_folder_postfix='own_dataset' model.vqgan_ckpt=<INSERT_PATH_TO_CHECKPOINT> model.results_folder=<INSERT_PATH_TO_OUTPUT_FOLDER> diffusion_img_size=128 model.diffusion_depth_size=8 model.diffusion_num_channels=8 model.dim_mults=[1,2,4,8] model.batch_size=2 model.gpus=1
```
